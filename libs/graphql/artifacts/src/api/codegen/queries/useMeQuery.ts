...
export const useMePartialQuery = <Selector extends Pick<QuerySelector, 'me'>['me']>(
  selector: Selector, 
  options?: Omit<
  UseQueryOptions<
    MeResult<Selector>,
...
type MeSelector = {
   id: boolean; email: boolean; lastName: boolean; firstName: boolean; joinDate: boolean; role: boolean; token: boolean;  
};
...
export const useMeQuery = (
  ...
  useMePartialQuery(
    {
       id: true, email: true, lastName: true, firstName: true, joinDate: true, role: true, token: true,  
    }
    ,options
  );

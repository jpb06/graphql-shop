...
type CategoriesSelectorResult = Pick<QuerySelectorResult, 'categories'>['categories'];

export type CategoriesResult<Selector> = {
  categories: DeepReplace<Selector, CategoriesSelectorResult>;
};

export const useCategoriesPartialQuery = <Selector extends Pick<QuerySelector, 'categories'>['categories']>(
  selector: Selector, 
  options?: Omit<
  UseQueryOptions<
...
>
  ...
  return useQuery<CategoriesResult<Selector>, unknown, CategoriesResult<Selector>>({
    queryKey: ['categories'],

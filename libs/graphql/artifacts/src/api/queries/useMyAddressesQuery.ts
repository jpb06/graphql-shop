...
type MyAddressesSelectorResult = Pick<QuerySelectorResult, 'myAddresses'>['myAddresses'];

export type MyAddressesResult<Selector> = {
  myAddresses: DeepReplace<Selector, MyAddressesSelectorResult>;
};

export const useMyAddressesPartialQuery = <Selector extends Pick<QuerySelector, 'myAddresses'>['myAddresses']>(
  selector: Selector, 
  options?: Omit<
  UseQueryOptions<
...
>
  ...
  return useQuery<MyAddressesResult<Selector>, unknown, MyAddressesResult<Selector>>({
...
type MyAddressesSelector = {
   id: boolean; street: boolean; zipCode: boolean; city: boolean; country: boolean;  
};
...
export const useMyAddressesQuery = (
  ...
  useMyAddressesPartialQuery(
    {
       id: true, street: true, zipCode: true, city: true, country: true,  
    }
    ,options
  );
